...
class LogTable(DataTable):
    async def on_mouse_down(self, event: events.MouseDown) -> None:
        if self.app and hasattr(self.app, "handle_log_mouse_down"):
             try:
                 meta = self.get_cell_at(event.offset)
                 if meta:
                      row_key = self.coordinate_to_cell_key(meta).row_key
                      if row_key:
                           if self.app.handle_log_mouse_down(row_key.value, shift=event.shift):
                                self.capture_mouse()
    ...
    async def on_mouse_move(self, event: events.MouseMove) -> None:
        if getattr(self, "_mouse_captured", False) and self.app and hasattr(self.app, "handle_log_mouse_move"):
             try:
                 meta = self.get_cell_at(event.offset)
                 if meta:
                      row_key = self.coordinate_to_cell_key(meta).row_key
...
class PyWireDevDashboard(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Label(f"Server: http://{self.host}:{self.port} | Uptime: 00:00:00", id="header-info")
    ...
    async def on_mount(self) -> None:
        ...
        self.log_write(f"[bold yellow]Initializing pywire Server on {protocol}://{self.host}:{self.port}...[/]", level=20)
    ...
    def update_uptime(self) -> None:
        ...
        header_info.update(f"Server: {protocol}://{self.host}:{self.port} | Uptime: {uptime_str} | Log Level: {self.current_log_level}")
    ...
    def action_toggle_log_level(self):
        self.current_log_level_index = (self.current_log_level_index + 1) % len(self.log_levels)
        ...
        async def read_stream(stream):
            while True:
                ...
                if "Press CTRL+C to quit" in text_content:
                    text_content = text_content.replace("Press CTRL+C to quit", "Press q to quit")
                
                # Determine level
                level = 20 # Default INFO
                upper_text = text_content.upper()
                if "DEBUG" in upper_text: level = 10
                elif "INFO" in upper_text: level = 20
                elif "WARNING" in upper_text: level = 30
                elif "ERROR" in upper_text: level = 40
                
                # System/Always visible messages logic
                if "PyWire" in text_content:
                     if "Error" in text_content: level = 40
                     elif "Warning" in text_content: level = 30
                     else: level = 20
        ...
        if self._selected_indices:
             ...
             for idx in sorted_indices:
                  if 0 <= idx < len(self._log_store):
                       ...
                       if "Tip: Use Space or Click" in plain: continue
                       lines.append(plain)
        else:
             ...
             for text_obj, level in self._log_store:
                  ...
                  plain = text_obj.plain
                  if "Copied" in plain and "lines" in plain and "clipboard" in plain: continue
                  if "Tip: Use Space or Click" in plain: continue
...
def start_tui(app_path: str, host: str, port: int, ssl_keyfile: str | None, ssl_certfile: str | None, env_file: str | None) -> None:
    cmd = [sys.executable, "-m", "pywire.cli.main", "dev", app_path, "--no-tui", "--host", host, "--port", str(port)]
