...
async def run_dev_server(
    ...
    if not pages_dir.exists():
        console.print(f"[bold yellow]Warning[/]: Pages directory '{pages_dir}' does not exist.")
    ...
    async def watch_changes() -> None:
        try:
            ...
            console.print(f"[bold cyan]PyWire[/]: Watching [bold]{pages_dir}[/] for changes...")
    ...
    if not cert_path or not key_path:
        ...
        for c_file, k_file in potential_certs:
            if c_file.exists() and k_file.exists():
                console.print(f"[bold cyan]PyWire[/]: Found local certificates ([bold]{c_file.name}[/]), using them.")
        ...
        if not found:
            ...
            if shutil.which("mkcert"):
                ...
                try:
                    ...
                    console.print(f"[bold cyan]PyWire[/]: Certificates generated ({pem_file.name}).")
                    console.print(
    ...
    async with asyncio.TaskGroup() as tg:
        if has_http3:
            try:
                ...
                console.print(f"[bold red]PyWire Error[/]: Failed to start Hypercorn: {e}")
                import traceback

                traceback.print_exc()
                console.print("[bold yellow]PyWire[/]: Falling back to Uvicorn (HTTP/2 + WebSocket only)")
                has_http3 = False
        ...
        if not has_http3:
            ...
            console.print(f"[bold cyan]PyWire[/]: Running on [bold cyan]{protocol}://{host}:{port}[/]")
