...
> **Design decisions (orchestrator, 2026-09-24):** `.while={expr}` is **server-evaluated** — the client cannot run Python; codegen evaluates the expression on every render and emits its truthiness as `data-pw-poll-while="true|false"`; the client starts polling only if true-on-mount and stops when it morphs to false (cost: one extra round-trip after the condition flips — acceptable, and it keeps one source of truth). `.every` is clamped at compile time: `< 100 ms` → compile error (billing foot-gun on FaaS). Overlap guard: one in-flight poll per element (skip tick while a dispatch is pending).
