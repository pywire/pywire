...
The full syntax is **`{$for item in collection, key=expr}`** — note the **comma** before `key=`, and use **bracket** access (`item['id']`) rather than `item.id` when iterating dicts.
