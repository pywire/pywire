...
> **`{$await}` is stateful-only.** It needs a server-owned timeline to push the resolved view, so `pywire build` rejects it in `PyWire(stateless=True)` apps. On stateless or HTTP-only deployments, write the operation to a durable store and update it with [`@poll`](/reference/poll-directive/).
...
`{$then name}` **binds the resolved value** to the local `name` so the body can reference it. The order is fixed: pending body comes first, then `{$then variable_name}`, then the resolved body.
