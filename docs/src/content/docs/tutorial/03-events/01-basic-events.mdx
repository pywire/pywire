...
Events behave like standard DOM events. To suppress the browser's default behavior — e.g. on `<form>` submits — use the `.prevent` modifier (`@submit.prevent={save}`), covered in the modifiers step.
