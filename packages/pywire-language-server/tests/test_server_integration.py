...
"""
    ...
    params = HoverParams(text_document=TextDocumentIdentifier(uri=uri), position=pos)
    ...
    assert (
        "int" in result.contents
        or "10" in result.contents
        or "my_var" in result.contents
    )
...
"""
    ...
    params = CompletionParams(
        text_document=TextDocumentIdentifier(uri=uri), position=pos, context=None
    )
...
"""
    ...
    res_block = await hover(
        mock_ls,
        HoverParams(text_document=TextDocumentIdentifier(uri=uri), position=pos_block),
    )
    ...
    res_attr = await hover(
        mock_ls,
        HoverParams(text_document=TextDocumentIdentifier(uri=uri), position=pos_attr),
    )
...
async def test_validate_blocks(mock_ls, clean_documents):
    ...
    assert any(
        "Mismatched closing tag: expected {/if}, got {/for}" in d.message
        for d in diags3
    )
...
async def test_completions_improved(mock_ls, clean_documents):
    ...
    res = await completions(
        mock_ls,
        CompletionParams(text_document=TextDocumentIdentifier(uri=uri), position=pos),
    )
    ...
    res2 = await completions(
        mock_ls,
        CompletionParams(text_document=TextDocumentIdentifier(uri=uri), position=pos2),
    )
...
"""
    ...
    _res_expr = await completions(
        mock_ls,
        CompletionParams(
            text_document=TextDocumentIdentifier(uri=uri), position=pos_expr
        ),
    )
