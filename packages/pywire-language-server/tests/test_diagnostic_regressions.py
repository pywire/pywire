...
@pytest.mark.skip(
    reason="Transpiler replaces invalid Python with error docstring; mapping test needs valid syntax"
)
@pytest.mark.asyncio
...
"""
    ...
    assert def_foo_line_idx != -1, (
        f"Could not find 'def foo' in generated code: {gen_lines!r}"
    )
    ...
    if mapped_diag_loose is None:
        pytest.fail(
            "Strict mapping dropped diagnostic at line end (col 9). Fuzzy mapping needed."
        )
...
def test_virtual_file_manager_uri_parsing() -> None:
    ...
    assert orig_b is not None, (
        "VirtualFileManager failed to resolve URI without file:// scheme"
    )
