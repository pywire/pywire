...
def _escape_html_part(html_part: str) -> str:
    ...
    html_part = re.sub(
        r"(?<=[\s<])@([a-zA-Z0-9._-]+)(?==|\{)", r"__pw_on_\1", html_part
    )
    html_part = re.sub(
        r"(?<=[\s<])\$([a-zA-Z0-9._-]+)(?==|\{)", r"__pw_dir_\1", html_part
    )
    return html_part
...
async def handle_js_message(event_data):
    try:
        ...
        if event_type == "http_request":
            if DEBUG_SHIM:
                print(
                    f"DEBUG: HTTP Request: {event_data['method']} {event_data['path']}"
                )
            ...
            scope = {
                ...
                "headers": [
                    ...
                    for k, v in event_data["headers"].items()
                ],
                "server": ("localhost", 80),
                "client": ("client", 0),
            }
            ...
            def send_to_js(msg):
                ...
                js_payload = to_js(
                    response_payload, dict_converter=js.Object.fromEntries
                )  # ty: ignore
            ...
            scope = {
                ...
                "headers": [
                    (k.lower().encode(), v.encode())
                    for k, v in event_data.get("headers", {}).items()
                ],
                "server": ("localhost", 80),
                "client": ("client", 0),
            }
            ...
            def send_to_js(msg):
                ...
                if msg.get("type") == "websocket.close":
                    if DEBUG_SHIM:
                        print(
                            f"DEBUG: WebSocket closed by app. Code: {msg.get('code')}"
                        )
                ...
                if (
                    msg.get("type") == "websocket.send"
                    and "bytes" in msg
                    and isinstance(msg["bytes"], bytes)
                ):
                    try:
                        import msgpack
                        ...
                        if isinstance(decoded_payload, dict):
                            if (
                                decoded_payload.get("type") == "update"
                                and "html" in decoded_payload
                            ):
                                ...
                                if "<html" in lower_html or "<!doctype" in lower_html:
                                    ...
                                    if not body_match:
                                        ...
                                        if len(parts) > 1:
                                            ...
                                            content = parts[1]
                                            content = re.split(
                                                r"</body>", content, flags=re.IGNORECASE
                                            )[0]
                                            decoded_payload["html"] = content
                                            is_modified = True
                                            if DEBUG_SHIM:
                                                print(
                                                    "DEBUG: Stripped full HTML wrapper (fallback method)"
                                                )
                                    ...
                                    if body_match and not is_modified:
                                        ...
                                        if DEBUG_SHIM:
                                            print(
                                                "DEBUG: Stripped full HTML wrapper from Server->Client update payload"
                                            )
                ...
                response_payload = {"type": "ws_message", "id": req_id, "message": msg}
            ...
            if queue:
                ...
                if isinstance(data, list):
                    ...
                    queue.put_nowait({"type": "websocket.receive", "bytes": data_bytes})
                else:
                    # Legacy text mode
                    if DEBUG_SHIM:
                        print(
                            f"DEBUG: ws_send text data length: {len(data) if data else 0}"
                        )
...
def reload_page(path_str):
    ...
    try:
        ...
        if path.exists():
            ...
            if DEBUG_SHIM:
                print(
                    f"DEBUG: Reloading {path_str}, but file DOES NOT EXIST at that path!"
                )
