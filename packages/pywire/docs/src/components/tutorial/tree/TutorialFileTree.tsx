...
export const TutorialFileTree: React.FC<TutorialFileTreeProps> = ({
  ...
  const renderNode = (node: FileNode, depth: number) => {
    ...
    const isDeletable =
      meta?.deletable === true ||
      (behaviors?.canDeleteFiles?.some((p) => {
    ...
    // Folder Logic
    const canAddInFolder =
      behaviors?.canAddFiles &&
      Object.keys(behaviors.canAddFiles).some(
  ...
  const rootCanAdd =
    behaviors?.canAddFiles && Object.keys(behaviors.canAddFiles).some((p) => !p.includes('/'))

  return (
    <div
      className="pw-panel-bg"
      style={{ height: '100%', display: 'flex', flexDirection: 'column' }}
    >
      <div
        className="pw-filetree-header"
