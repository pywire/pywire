...
def pywire_server(test_app_dir, server_port):
    ...
    import sys
    cmd = [
        sys.executable, "-m", "pywire.cli.main", "dev", 
        "app:app",  
        "--host", "127.0.0.1", 
            ...
            try:
                ...
                if chunk:
                    ...
                    if not url_found_event.is_set():
                        ...
                        match = re.search(r"Running on\s+.*?(https?://127.0.0.1:\d+)", full_output, re.DOTALL) or \
                                re.search(r"Uvicorn running on\s+.*?(https?://127.0.0.1:\d+)", full_output, re.DOTALL)
                        if match:
                            nonlocal url
    ...
    while time.time() - start_time < timeout:
        if url_found_event.is_set():
            break
        if process.poll() is not None:
             raise RuntimeError(f"Server exited prematurely.\nOutput so far: {''.join(captured_output)}")
        time.sleep(0.1)
    
    if not url:
         os.killpg(os.getpgid(process.pid), signal.SIGTERM)
         raise RuntimeError(f"Server timeout or URL not found.\nCaptured Output: {''.join(captured_output)}")
