...
def test_hot_reloading_issue_4(page: Page, pywire_server: str, test_app_dir: str):
    ...
    try:
        ...
        expect(page.locator("#child-content")).to_have_text(
            "Updated Text", timeout=10000
        )
    finally:
        # Restore the original file to keep the fixture clean for subsequent tests
        child_path.write_text(original_content)
        # Wait for the file watcher to detect and process the restore, so the
        # async reload doesn't leak into subsequent tests.
